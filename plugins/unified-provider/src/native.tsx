...
export function ImportWizard({ controller, registry, version, onClose }: { controller: ProviderController; registry: RegistryView; version: string | null; onClose: () => void }) {
  ...
  async function previewSource() {
    ...
    try {
      ...
      const official = await controller.loadTemplates({ engineId: result.target.engineId, executionTarget: { kind: "local" } }, true);
      setPreview(result); setTemplates(official); setDecisions(result.candidates.map((c) => ({ candidateId: c.candidateId, action: (c.hasStaticKey || c.auth === "none") && c.models.length && compatible(result.target.engineId, c.protocol) ? "new" : "skip", engineId: result.target.engineId, importKey: c.hasStaticKey }))); setStep(2);
  ...
  return <section className="up-card up-wizard" aria-label="导入原生 CLI 配置">
    ...
    {step === 2 && preview && <><p>{preview.target.label} · 识别到 {preview.candidates.length} 个候选项。名称或 URL 相同并不代表是同一个账号。</p>{preview.warnings.map((warning) => <p className="up-muted" key={warning}>{warning}</p>)}
      {preview.candidates.map((candidate) => { const decision = decisions.find((d) => d.candidateId === candidate.candidateId)!; return <fieldset className="up-card" key={candidate.candidateId}><legend>{candidate.label}</legend>
        ...
        {decision.action !== "skip" && <label className="up-check"><input type="checkbox" checked={decision.importKey} disabled={!candidate.hasStaticKey} onChange={(event) => revise(candidate.candidateId, { importKey: event.target.checked })} />把该候选项的静态 Key 复制到新端点{candidate.hasStaticKey ? "" : " · 此来源没有可复制的静态 Key"}</label>}
    ...
    {step === 3 && preview && <><h3>确认一次性复制</h3><ul>{decisions.filter((d) => d.action !== "skip").map((d) => <li key={d.candidateId}>{preview.candidates.find((c) => c.candidateId === d.candidateId)?.label}：{d.action === "new" ? "新建供应商" : `并入 ${registry.providers.find((p) => p.id === d.providerId)?.name}`} · 新端点 · {d.engineId} · {d.templateRef?.modelId} · Key {d.importKey ? "复制到该端点" : "稍后手动填写"}</li>)}</ul>
