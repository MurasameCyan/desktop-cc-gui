...
function context(permissions: string[], web = false) {
  client.web = web;
  const unexpected = vi.fn(async () => { throw new Error("backend must not be reached"); });
  const backend: PluginContextBackend = {
    get: async () => null,
    set: async () => {},
    delete: async () => {},
    bridgeInvoke: unexpected,
    workspaceMetadata: unexpected,
    workspaceList: unexpected,
    pickDirectory: unexpected,
    documentStorageGetLocation: unexpected,
    documentStorageSelectLocation: unexpected,
    documentStorageReadText: unexpected,
    documentStorageWriteTextAtomic: unexpected,
    documentStorageRemove: unexpected,
    documentStorageList: unexpected,
  };
