...
describe("computer-use sends", () => {
  ...
  it("keeps the flag on a queued computer-use turn", async () => {
    ...
    // Drain it the way the queue does on user request. The drain is
    // fire-and-forget and the send now resolves the session's authoritative
    // selection first, so the spawn lands a few microtasks later.
