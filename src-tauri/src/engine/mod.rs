...
pub(crate) use events::{
    ...
    safe_prompt_arg, tool_call_message, tool_call_message_with_id, tool_call_patch_with_id,
...
pub async fn send_selected_message(state: &crate::AppState, request: SendMessageRequest) -> Result<SendResult, String> {
    ...
    send_message_inner_with_sink(state, Arc::clone(&state.sink), request.target.engine_id.clone(),
        ...
        request.prompt_contributions, request.image_paths, model, input.effort, request.permission, provider_id, request.run_id,
        request.computer_use, execution, Some(context), None).await
}
fn prepare_launch(
    engine: &str,
    workspace_path: &str,
...
async fn send_reserved(
    ...
    if launch.engine_impl.transport_for(wsl_tp.is_some()) == Transport::Own {
        ...
        if engine == "codex"
            && launch.req.execution.is_none()
            && !codex_read_only::requested(&launch.req)
        {
