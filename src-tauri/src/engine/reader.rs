...
pub fn sweep_staging_dirs() {
    ...
    for name in ["claude-staging", "grok-staging", "kimi-staging", "provider-staging"] {
...
mod staging_tests {
    ...
    async fn reader_context_cleans_private_configs_on_completion_and_abort() {
        for abort in [false, true] {
            ...
            let ctx = RunContext { deferred_prompt: None, core: TurnCore { execution: None, sink: event_sink::EventSink::new(Arc::new(Noop)), registry: Arc::new(ProcessRegistry::default()), engine_id: "grok".into(), run_id: "test".into() },
...
mod terminal_event_tests {
    ...
    async fn terminal_runs_do_not_emit_late_usage_text_or_duplicate_completion() {
        for fail in [false, true] {
            ...
            let core = TurnCore { execution: None, sink: event_sink::EventSink::new(collector.clone()),
