...
- 共通兼容线以 `0.3.12` 重新编号，CCB 与 Live2D 插件声明 `sdkVersion: ">=0.3.12"`：这是最低 SDK 版本，不是精确版本锁；`0.3.16` 及后续更高版本可通过握手，低于 `0.3.12` 仍会被拒绝。宿主仍须提供插件依赖的通用能力和权限；来源版本标注不等于本线的 manifest 版本，也不能凭能力存在跳过其他插件声明的版本范围。
